...
class PGLearner:
    ...
    def write_net_to_nnet(self, outputFileName):
        # print ("net params:")
        params_s = lasagne.layers.helper.get_all_params(self.l_out)
        # print(params_s)
        params = self.return_net_params()
        input_size = params[0].shape[0]
        weights = [params[0],params[0]]
        ...
        #     print(item)
        #     print(item.shape)
        inputMins = [0.0] * input_size
        inputMaxes = [1.0] * input_size
        means = [0.0] * (input_size + 1)
        means[-1] = 2.5
        ranges = [0.0] * (input_size + 1)
        ranges[-1] = 5
        # exit(333)
        writeNNet.writeNNet(weights, biases, inputMins, inputMaxes, means, ranges, outputFileName+'.nnet')
