...
class Dist(object):
    ...
    def constant_dist(self, length=20, nw_size=np.full((2,), 10, dtype=u'int32')):
        return length, nw_size
...
def generate_sequence_work(pa, seed=42):
    ...
    nw_size_seq = np.reshape(nw_size_seq,
                             [pa.num_ex, pa.simu_len, pa.num_resources])
