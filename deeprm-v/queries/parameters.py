...
class Parameters(object):
    def __init__(self):
        ...
        self.output_filename = u'data/tmp'

        self.num_epochs = 10000         # number of training epochs
        self.simu_len = 66             # length of the busy cycle that repeats itself
        ...
        self.num_resources = 2               # number of resources in the system
        ...
        self.dist = job_distribution.Dist(
            self.num_resources, self.max_job_size, self.max_job_len).bi_model_dist
        ...
        print("backlog_width = ", self.backlog_width)
        print("backlog_size = ", self.backlog_size)
        self.network_input_height = self.time_horizon
        print("network_input_height = ", self.network_input_height)
        self.network_input_width = \
            (self.res_slot +
             self.max_job_size * self.num_nw) * self.num_resources + \
            self.backlog_width + \
            1  # for extra info, 1) time since last new job
        print("network_input_width = ", self.network_input_width)
        # exit(0)
        # compact representation
        self.network_compact_dim = (self.num_resources + 1) * \
        ...
        self.batch_size = 10
        self.evaluate_policy_name = u"SJF"
    ...
    def compute_dependent_parameters(self):
        ...
        self.network_input_width =int( \
            (self.res_slot + self.max_job_size * self.num_nw) * self.num_resources + \
            self.backlog_width + 1)  # for extra info, 1) time since last new job
