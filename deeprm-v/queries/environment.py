...
import math
import matplotlib
matplotlib.use('TkAgg', True, True)
...
class Env(object):
    def __init__(self, pa, nw_len_seqs=None, nw_size_seqs=None,
        ...
        if nw_len_seqs is None or nw_size_seqs is None:
            ...
            self.workload = np.zeros(pa.num_resources)
            for i in range(pa.num_resources):
                self.workload[i] = \
            ...
            self.nw_size_seqs = np.reshape(self.nw_size_seqs,
                                           [self.pa.num_ex, self.pa.simu_len, self.pa.num_resources])
        else:
    ...
    def generate_sequence_work(self, simu_len):
        ...
        nw_size_seq = np.zeros((simu_len, self.pa.num_resources), dtype=int)
    ...
    def observe(self):
        if self.repre == 'image':
            ...
            for i in range(self.pa.num_resources):
                ...
                for j in range(self.pa.num_nw):
                    ...
                    if self.job_slot.slot[j] is not None:  # fill in a block of work
                        # print(image_repr[: self.job_slot.slot[j].len, ir_pt: ir_pt + self.job_slot.slot[j].res_vec[i]])
            ...
            compact_repr = np.zeros(self.pa.time_horizon * (self.pa.num_resources + 1) +  # current work
                                    self.pa.num_nw * (self.pa.num_resources + 1) +        # new work
                                    1,                                              # backlog indicator
            ...
            for i in range(self.pa.num_nw):
                ...
                if self.job_slot.slot[i] is None:
                    compact_repr[cr_pt: cr_pt + self.pa.num_resources + 1] = 0
                    cr_pt += self.pa.num_resources + 1
                else:
                    compact_repr[cr_pt] = self.job_slot.slot[i].len
                    cr_pt += 1

                    for j in range(self.pa.num_resources):
    ...
    def plot_state(self):
        ...
        for i in range(self.pa.num_resources):
            ...
            plt.subplot(self.pa.num_resources,
        ...
        plt.subplot(self.pa.num_resources,
                    1 + self.pa.num_nw + 1,  # first +1 for current work, last +1 for backlog queue
                    self.pa.num_resources * (self.pa.num_nw + 1) + skip_row + 1)  # plot the backlog at the end, +1 to avoid 0
        ...
        if status == 'MoveOn':
            ...
            if not done:
                ...
                if self.seq_idx < self.pa.simu_len:  # otherwise, end of new job sequence, i.e. no new jobs
                    new_job = self.get_new_job_from_seq(self.seq_no, self.seq_idx-1)
...
class JobSlot(object):
    def __init__(self, pa, jobSlot = None):
        if jobSlot == None:
...
class Machine(object):
    ...
    def __init__(self, pa):
        ...
        self.canvas = np.zeros((pa.num_resources, pa.time_horizon, pa.res_slot))

    def allocate_job(self, job, curr_time):

        allocated = False

        for t in range(0, self.time_horizon - job.len  + 1):
