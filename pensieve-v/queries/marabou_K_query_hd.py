...
def k_test(filename,k,download_time):
    ...
    for j in range(k):
        ...
        for var in last_chunk_bit_rate[j]:
            if j == 0:
                l = utils.VIDEO_BIT_RATE[1]/utils.VIDEO_BIT_RATE[-1] # - MARABOU_ERR # default for the first chunk
                u = utils.VIDEO_BIT_RATE[1]/utils.VIDEO_BIT_RATE[-1] # + MARABOU_ERR # default for the first chunk
                network.setLowerBound(var, l)
                network.setUpperBound(var, u)
            else:
                l = utils.VIDEO_BIT_RATE[QUERY_BITRATE]/utils.VIDEO_BIT_RATE[-1] # - MARABOU_ERR # highest definition
                u = utils.VIDEO_BIT_RATE[QUERY_BITRATE]/utils.VIDEO_BIT_RATE[-1] # + MARABOU_ERR # highest definition
                network.setLowerBound(var, l)
                network.setUpperBound(var, u)
