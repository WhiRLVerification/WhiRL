...
class MarabouNetworkTF(MarabouNetwork.MarabouNetwork):
    ...
    def getValues(self, op):
        ...
        if op.node_def.op in ['MatMul', 'BiasAdd', 'Add', 'Sub', 'Relu', 'MaxPool', 'Conv2D', 'Placeholder','Mul','Maximum','SparseTensorDenseMatMul']:
    ...
    def mulEquations(self, op):
        ...
        assert (len(prevVars1) == len(prevVars2) )or (len(prevVars1) == 1) or len(prevVars2) == 1
        assert (len(curVars) == len(prevVars1)) or (len(curVars) == len(prevVars2))
        if len(prevVars1) == 1:
            prevVars1 = [prevVars1[0] for i in range(len(prevVars2)) ]
        if len(prevVars2) == 1:
            prevVars2 = [prevVars2[0] for i in range(len(prevVars1)) ]
    ...
    def maximumEquations(self, op):
        """
        Function to generate maximum equations
        Arguments:
            op: (tf.op) representing maximum operation
        """
        ### Get variables and constants of inputs ###
        input_ops = [i.op for i in op.inputs]
        prevValues = [self.getValues(i) for i in input_ops]
        ...
        for i in range(curValues.shape[1]):
            ...
            for x in prevValues:
                maxVars.add(x[0][i])
            self.addMaxConstraint(maxVars, curValues[0][i])
    ...
    def makeNeuronEquations(self, op):
        ...
        if op.node_def.op == 'MatMul':
            ...
            self.maximumEquations(op)
        elif  op.node_def.op == 'SparseTensorDenseMatMul':
            self.sparseTensorDenseMatMulEquations(op)
        else:
